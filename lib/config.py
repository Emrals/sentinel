...
sentinel_cfg = EmralsConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_emralsd_proto_version_with_sentinel_ping = 70207


def get_emrals_conf():
    if sys.platform == 'win32':
        emrals_conf = os.path.join(os.getenv('APPDATA'), "EmralsCore/emrals.conf")
    else:
        home = os.environ.get('HOME')

        emrals_conf = os.path.join(home, ".emralscore/emrals.conf")
        if sys.platform == 'darwin':
            emrals_conf = os.path.join(home, "Library/Application Support/EmralsCore/emrals.conf")

    emrals_conf = sentinel_cfg.get('emrals_conf', emrals_conf)

...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, emralsd):
        golist = emralsd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_emralsd(emralsd, item)
            except Exception as e:
    ...
    def vote_delete(self, emralsd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(emralsd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, emralsd, signal, outcome):
        import emralslib
        ...
        output = emralsd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = emralslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(emralsd, signal)

    def sync_network_vote(self, emralsd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = emralsd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = EMRALSD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 emrals addr, non-multisig
            if not emralslib.is_valid_emrals_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Emrals address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Emrals Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (emralslib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = EMRALSD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not emralslib.is_valid_emrals_address(addr, config.network):
    ...
    def hash(self):
        import emralslib
        return emralslib.hashit(self.serialise())

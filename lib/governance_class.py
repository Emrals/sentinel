...
class GovernanceClass(object):
    ...
    def vote(self, emralsd, signal, outcome):
        return self.go.vote(emralsd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, emralsd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(emralsd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(emralsd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import emralslib
        obj_data = emralslib.SHIM_serialise_for_emralsd(self.serialise())
    ...
    def emralsd_serialise(self):
        import emralslib
        return emralslib.SHIM_serialise_for_emralsd(self.serialise())

...
def is_valid_emrals_address(address, network='mainnet'):
    ...
    emrals_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Emrals address
        return False

    if (address_version != emrals_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.emralsd_serialise())
...
def SHIM_serialise_for_emralsd(sentinel_hex):
    ...
    # superblock => "trigger" in emralsd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # emralsd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    emralsd_hex = serialise(obj)
    return emralsd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('emrals.conf')

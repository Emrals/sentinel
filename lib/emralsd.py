...
class EmralsDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some emralsd methods
    ...
    @classmethod
    def from_emrals_conf(self, emrals_dot_conf):
        from emrals_config import EmralsConfig
        config_text = EmralsConfig.slurp_config_file(emrals_dot_conf)
        creds = EmralsConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = emralslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = emralslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = emralslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_emralsd_proto_version_with_sentinel_ping)

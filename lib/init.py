...
def has_emrals_conf():
    ...
    valid_emrals_conf = False

    # ensure emrals_conf exists & readable
    #
    # if not, print a message stating that Emrals Core must be installed and
    # configured, including JSONRPC access in emrals.conf
    try:
        f = io.open(config.emrals_conf)
        valid_emrals_conf = True
    except IOError as e:
        print(e)

    return valid_emrals_conf
...
def main():
    ...
    if not has_emrals_conf():
        print("EmralsCore must be installed and configured, including JSONRPC access in emrals.conf")
        sys.exit(1)
